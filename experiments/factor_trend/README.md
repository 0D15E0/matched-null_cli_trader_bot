...
- **The 2024+ window was never used by `factor_trend`.** A single
  pre-registered look would be permitted by the private holdout ledger; it is **not
...
- **A slip, self-reported.** One ad-hoc shell loop comparing the *incumbents*
  ...
  (the private holdout ledger), so no clean data was spent, and the numbers were
